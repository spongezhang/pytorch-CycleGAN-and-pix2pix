    ...
    def __getitem__(self, index):
        ...
        if self.resize_blur: 
            ...
            if resize_flag<0.5:
                random_size = random.uniform(0.5,1.0)
                w, h = B_img.size
                B_img = B_img.filter(ImageFilter.GaussianBlur(radius=0.5/random_size))
                B_img = B_img.resize((int(w*random_size),int(h*random_size)), Image.BICUBIC)
                B_img = B_img.resize((w,h), Image.BICUBIC)
